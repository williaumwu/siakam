def run(instructargs):
    ...
    if isinstance(stack.commit_info,str) or isinstance(stack.commit_info,unicode): 
        try:
    ...
    stack.logger.aggmsg(type(stack.commit_info))
    stack.logger.aggmsg("")
    ...
    inserts = stack.add_dict2dict(keys2pass,{},stack.commit_info)

    commit_url = stack.commit_info.get("url")
    if commit_url: inserts["commit_url"] = commit_url

    compare_url = stack.commit_info.get("compare")
    if compare_url: inserts["compare_url"] = compare_url

    repo_branch = stack.commit_info.get("branch")
    if repo_branch: inserts["repo_branch"] = repo_branch
    ...
    stack.run_metadata(inserts,publish=True)
    ...
    if commit_url: inserts["url"] = commit_url
    if compare_url: inserts["compare"] = compare_url
    if not inserts.get("repo_ref"): inserts["repo_ref"] = inserts["commit_hash"]
    if not inserts.get("sha"): inserts["sha"] = inserts["commit_hash"]
